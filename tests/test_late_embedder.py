...
def test_token_length_calculation(embedder: LateEmbedder):
    prefix_length, leading_special, trailing_special = embedder._get_token_lengths()

    # Check prefix length (for "Document: ")
    assert prefix_length == 2  # "Document" and ":" tokenize to 2 tokens

    #  MiniLM uses [CLS] at start and [SEP] at end
    assert leading_special == 1
    assert trailing_special == 1


def test_short_text_embedding(embedder: LateEmbedder):
...
def test_long_text_embedding(embedder: LateEmbedder):
...
def test_chunk_mapping(embedder):
    ...
    tokens = embedder.embedder.tokenizer(
        text, return_offsets_mapping=True, return_tensors="pt", add_special_tokens=False
    )
    ...
    # TODO: check that the token_chunks actually map to the original chunks
